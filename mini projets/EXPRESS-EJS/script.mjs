...
fs.writeFileSync(`./app/data/data.js`, `
const users = [
    {
        firstname: 'Emeric',
        lastname: 'Mathis',
        phone: '0702022205',
        password: 'emericmathis@gmail.com',
    ...
    },
    {
        ...
        address: '3 rue du CLos Marquet',
        cp: '43400',
        city: 'Saint-Chaond',
        password: '#-LOL-du42400'
    },
    {
        firstname: 'Nadjim',
        lastname: 'Kaabache',
...
console.log(` -- on génère quelques utilisateurs avec des mots de passe sécurisé --`);

import users from './app/database/data.js'
import client from './app/database/db.js';

users.map(user => {
    const request = 
    `
    INSERT INTO "User"(firstname, lastname, phone, email, password, address, cp, city)
    VALUES($1, $2, $3, $4, $5, $6, $7, $8);
    `;
    const hash = bcrypt.hash(user.password, 10);
    const values = [user.firstname, user.lastname, user.phone, user.email, hash, user.address, user.cp, user.city];
    const result = client.query(request, values);
})
